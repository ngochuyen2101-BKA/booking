	...
	function wc_thanks_redirect_settings( $settings, $current_section ) {
		...
		if ( $current_section === 'wctr' ) {
			...
			$settings_url[] = array(
				...
				'desc_tip' => __( 'WPML URLs is a PAID Feature. Please upgrade to <a href="https://nitin247.com/plugin/woocommerce-thanks-redirect-pro/">PRO</a>', 'wc-thanks-redirect-pro' ),
				'id'       => 'wctr_wpml_active',
				'type'     => 'checkbox',
				'default'  => 'no',
...
function wc_thanks_redirect_short_code_order_details(  ) {
	...
	$order_id = !empty($_GET['order_id']) ? wp_kses_post($_GET['order_id']) : 0; // phpcs:ignore WordPress.Security.NonceVerification.Recommended

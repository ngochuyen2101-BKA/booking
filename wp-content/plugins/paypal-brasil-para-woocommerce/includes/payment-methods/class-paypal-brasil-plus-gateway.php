...
class PayPal_Brasil_Plus_Gateway extends PayPal_Brasil_Gateway {
	...
	public function process_payment( $order_id, $force = false ) {
		...
		if ( isset( $_POST['wc-ppp-brasil-error'] ) && ! empty( $_POST['wc-ppp-brasil-error'] ) ) {
			...
			$order->add_order_note( "O pagamento não pode ser processado pelo PayPal: <b>IFRAME_$error_type</b>");
		...
		try {
			...
			$order->add_order_note( "Houve um erro ao executar o pagamento pelo PayPal: <b>EXECUTE_$error_type</b>" );
			$order->update_status( 'wc-failed' );
			$order->save();
